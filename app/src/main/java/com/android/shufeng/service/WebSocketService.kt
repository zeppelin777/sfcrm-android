package com.android.shufeng.service
...
import com.android.shufeng.activity.LoginActivity
import com.android.shufeng.activity.MainActivity
import com.android.shufeng.base.UrlUtils
import com.android.shufeng.bean.*
import com.android.shufeng.bean2.NewSocketBean
import com.android.shufeng.net.HttpRequest
import com.android.shufeng.net.NetResponseCallBack
import com.android.shufeng.utils.*
...
//        showLoading()
        ...
        val params = CallTimeParams().apply {
            ...
            recordFile = getRecordFile(callLogBean.phoneNumber, answerTime)
            if (recordFile != null && recordFile!!.exists()) {
                music_url = recordFile!!.name
            }
        ...
        HttpRequest.getInstance().postJson(UrlUtils.SendCallTimeUrl, Gson().toJson(params), this, object : NetResponseCallBack<SaveRecordBean>(this) {
            override fun onSuccessObject(data: SaveRecordBean?, id: Int) {
                ...
                if (recordFile != null && recordFile!!.exists()) {
                    uploadRecord2(recordFile!!)
                } else {
                    // 错误，没有找到录音文件
                    Intent().apply {
                        action = Constants.RECEIVER_ACTION
                        putExtra("type", Constants.ACTION_SHOW_MSG)
                        putExtra("message", "错误，没有找到录音文件")
                        sendBroadcast(this)
                    }
                }
...
//                    {"code":401,"message":"token已失效"}
    ...
    private fun uploadRecord2(file: File) {
